...
export const SERVER_VERSION = '1.6.10';
...
const toolDefinitionMap: Map<string, McpToolDefinition> = new Map([
  ["PutV1InstancesById", {
    ...
    inputSchema: {"type":"object","properties":{"id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"name":{"type":"string","minLength":3,"maxLength":100},"customer_invite_redirect_url":{"type":["string","null"],"format":"uri"},"email_notifications":{"type":"boolean","default":true,"description":"Whether email notifications are enabled for this instance"},"require_passkey":{"type":"boolean","default":false,"description":"Whether passkey verification is required for protected actions"},"compliance_emails":{"type":["array","null"],"items":{"type":"string","format":"email"},"maxItems":20,"description":"When non-empty, compliance Request for Information emails are sent only to these addresses instead of all instance members"},"customer_rfi_emails_enabled":{"type":"boolean","description":"Whether customer (customer) Request for Information notification emails are sent for this instance. Omit to leave unchanged."}},"required":["name"],"description":"Update instance"}},"required":["id"]},
    method: "put",
    pathTemplate: "/v1/instances/{id}",
    executionParameters: [{"name":"id","in":"path"},{"name":"Idempotency-Key","in":"header"}],
  ...
  ["DeleteV1InstancesById", {
    ...
    inputSchema: {"type":"object","properties":{"id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."}},"required":["id"]},
    method: "delete",
    pathTemplate: "/v1/instances/{id}",
    executionParameters: [{"name":"id","in":"path"},{"name":"Idempotency-Key","in":"header"}],
  ...
  ["PutV1InstancesMembersByUserId", {
    ...
    inputSchema: {"type":"object","properties":{"id":{"type":"string"},"user_id":{"type":"string"},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"user_role":{"type":"string","enum":["owner","admin","finance","checker","operations","developer","viewer"]}},"required":["user_role"],"description":"Update instance member role"}},"required":["id","user_id"]},
    method: "put",
    pathTemplate: "/v1/instances/{id}/members/{user_id}",
    executionParameters: [{"name":"id","in":"path"},{"name":"user_id","in":"path"},{"name":"Idempotency-Key","in":"header"}],
    requestBodyContentType: "application/json",
    securityRequirements: [{"Bearer":[]}]
  }],
  ["DeleteV1InstancesMembersByUserId", {
    ...
    inputSchema: {"type":"object","properties":{"id":{"type":"string"},"user_id":{"type":"string"},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."}},"required":["id","user_id"]},
    method: "delete",
    pathTemplate: "/v1/instances/{id}/members/{user_id}",
    executionParameters: [{"name":"id","in":"path"},{"name":"user_id","in":"path"},{"name":"Idempotency-Key","in":"header"}],
    requestBodyContentType: undefined,
  ...
  ["PostV1InstancesOwnership", {
    ...
    inputSchema: {"type":"object","properties":{"id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"user_id":{"type":"string","minLength":15,"maxLength":15}},"required":["user_id"],"description":"Transfer instance ownership to another current member"}},"required":["id"]},
    method: "post",
  ...
  ["GetV1InstancesRfi", {
    name: "GetV1InstancesRfi",
    description: `Returns the open (pending) RFI for this instance, or null if the instance has no pending RFI.`,
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15}},"required":["instance_id"]},
    method: "get",
    pathTemplate: "/v1/instances/{instance_id}/rfi",
    executionParameters: [{"name":"instance_id","in":"path"}],
    requestBodyContentType: undefined,
    securityRequirements: [{"Bearer":[]}]
  }],
  ["PostV1InstancesRfi", {
    name: "PostV1InstancesRfi",
    description: `Submit a flat response object for the open instance RFI. Standalone: submitting does not change any instance or onboarding status. Returns \`{ success: false, reason }\` when no pending RFI exists, callers should refetch the RFI state instead of retrying.`,
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"requestBody":{"type":"object","additionalProperties":{},"description":"The JSON request body."}},"required":["instance_id"]},
    method: "post",
    pathTemplate: "/v1/instances/{instance_id}/rfi",
    executionParameters: [{"name":"instance_id","in":"path"}],
    requestBodyContentType: "application/json",
  ...
  ["PostV1EInstancesTos", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"requestBody":{"type":"object","properties":{"idempotency_key":{"type":"string","format":"uuid"},"customer_id":{"type":["string","null"],"minLength":15,"maxLength":15},"redirect_url":{"type":["string","null"],"format":"uri"}},"required":["idempotency_key"],"description":"Initiate a new terms of service"}},"required":["instance_id"]},
    method: "post",
  ...
  ["PostV1InstancesCustomersLimitIncrease", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"customer_id":{"type":"string","minLength":15,"maxLength":15,"description":"Customer ID"},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"per_transaction":{"type":["integer","null"],"maximum":100000000000},"daily":{"type":["integer","null"],"maximum":100000000000},"monthly":{"type":["integer","null"],"maximum":100000000000},"supporting_document_type":{"type":"string","enum":["individual_bank_statement","individual_tax_return","individual_proof_of_income","individual_pay_stub","individual_employment_letter","individual_investment_statement","individual_crypto_exchange_statement","individual_blockchain_wallet_statement","business_bank_statement","business_financial_statements","business_tax_return","business_contract","business_accounts_receivable","business_merchant_processor_statement","business_shareholder_loan"]},"supporting_document_file":{"type":"string","format":"uri"}},"required":["per_transaction","daily","monthly","supporting_document_type","supporting_document_file"],"description":"Request new limit increase"}},"required":["instance_id","customer_id"]},
    method: "post",
    pathTemplate: "/v1/instances/{instance_id}/customers/{customer_id}/limit-increase",
  ...
  ["GetV1InstancesCustomers", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"limit":{"type":"string","enum":["10","50","100","200","500","1000"],"description":"Number of items to return"},"offset":{"type":"string","enum":["0","10","50","100","200","500","1000"],"description":"Number of items to skip"},"starting_after":{"type":"string","description":"A cursor for use in pagination. starting_after is an object ID that defines your place in the list."},"ending_before":{"type":"string","description":"A cursor for use in pagination. ending_before is an object ID that defines your place in the list."},"full_name":{"type":"string"},"customer_name":{"type":"string"},"status":{"type":"string","enum":["verifying","approved","rejected","deprecated","pending_review","awaiting_contract","compliance_request","approved_rfi"]},"customer_id":{"type":"string"},"bank_account_id":{"type":"string"},"country":{"type":"string"}},"required":["instance_id"]},
    method: "get",
    pathTemplate: "/v1/instances/{instance_id}/customers",
    executionParameters: [{"name":"instance_id","in":"path"},{"name":"limit","in":"query"},{"name":"offset","in":"query"},{"name":"starting_after","in":"query"},{"name":"ending_before","in":"query"},{"name":"full_name","in":"query"},{"name":"customer_name","in":"query"},{"name":"status","in":"query"},{"name":"customer_id","in":"query"},{"name":"bank_account_id","in":"query"},{"name":"country","in":"query"}],
    requestBodyContentType: undefined,
    securityRequirements: [{"Bearer":[]}]
  }],
  ...
  ["PutV1InstancesCustomersById", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"email":{"type":"string","format":"email"},"tax_id":{"type":["string","null"]},"address_line_1":{"type":["string","null"]},"address_line_2":{"type":["string","null"]},"city":{"type":["string","null"]},"state_province_region":{"type":["string","null"]},"country":{"type":"string","enum":["AF","AL","DZ","AS","AD","AO","AI","AQ","AG","AR","AM","AW","AU","AT","AZ","BS","BH","BD","BB","BY","BE","BZ","BJ","BM","BT","BO","BQ","BA","BW","BV","BR","IO","BN","BG","BF","BI","CV","KH","CM","CA","KY","CF","TD","CL","CN","CX","CC","CO","KM","CD","CG","CK","CR","HR","CU","CW","CY","CZ","CI","DK","DJ","DM","DO","EC","EG","SV","GQ","ER","EE","SZ","ET","FK","FO","FJ","FI","FR","GF","PF","TF","GA","GM","GE","DE","GH","GI","GR","GL","GD","GP","GU","GT","GG","GN","GW","GY","HT","HM","VA","HN","HK","HU","IS","IN","ID","IR","IQ","IE","IM","IL","IT","JM","JP","JE","JO","KZ","KE","KI","KP","KR","KW","KG","LA","LV","LB","LS","LR","LY","LI","LT","LU","MO","MG","MW","MY","MV","ML","MT","MH","MQ","MR","MU","YT","MX","FM","MD","MC","MN","ME","MS","MA","MZ","MM","NA","NR","NP","NL","NC","NZ","NI","NE","NG","NU","NF","MP","NO","OM","PK","PW","PS","PA","PG","PY","PE","PH","PN","PL","PT","PR","QA","MK","RO","RU","RW","RE","BL","SH","KN","LC","MF","PM","VC","WS","SM","ST","SA","SN","RS","SC","SL","SG","SX","SK","SI","SB","SO","ZA","GS","SS","ES","LK","SD","SR","SJ","SE","CH","SY","TW","TJ","TZ","TH","TL","TG","TK","TO","TT","TN","TR","TM","TC","TV","UG","UA","AE","GB","UM","US","UY","UZ","VU","VE","VN","VG","VI","WF","EH","YE","ZM","ZW","AX"]},"postal_code":{"type":["string","null"]},"ip_address":{"type":["string","null"],"format":"ip"},"latitude":{"type":["number","null"],"minimum":-90,"maximum":90,"description":"Optional geolocation latitude collected from the customer. Forwarded to AiPrise and SEON for fraud analysis."},"longitude":{"type":["number","null"],"minimum":-180,"maximum":180,"description":"Optional geolocation longitude collected from the customer. Forwarded to AiPrise and SEON for fraud analysis."},"image_url":{"type":["string","null"],"format":"uri"},"phone_number":{"type":["string","null"]},"proof_of_address_doc_type":{"type":["string","null"],"enum":["UTILITY_BILL","BANK_STATEMENT","RENTAL_AGREEMENT","TAX_DOCUMENT","GOVERNMENT_CORRESPONDENCE"]},"proof_of_address_doc_file":{"type":["string","null"],"format":"uri","description":"Please upload one document that serve as proof of address, this needs to be the same physical address as the one provided above. Only PDF and image files are accepted (.jpg, .jpeg, .png, .pdf) with a maximum size of 3MB."},"first_name":{"type":["string","null"]},"last_name":{"type":["string","null"]},"date_of_birth":{"anyOf":[{"type":"string","format":"date-time"},{"type":"string"},{"type":"null"}]},"id_doc_country":{"type":["string","null"],"enum":["AF","AL","DZ","AS","AD","AO","AI","AQ","AG","AR","AM","AW","AU","AT","AZ","BS","BH","BD","BB","BY","BE","BZ","BJ","BM","BT","BO","BQ","BA","BW","BV","BR","IO","BN","BG","BF","BI","CV","KH","CM","CA","KY","CF","TD","CL","CN","CX","CC","CO","KM","CD","CG","CK","CR","HR","CU","CW","CY","CZ","CI","DK","DJ","DM","DO","EC","EG","SV","GQ","ER","EE","SZ","ET","FK","FO","FJ","FI","FR","GF","PF","TF","GA","GM","GE","DE","GH","GI","GR","GL","GD","GP","GU","GT","GG","GN","GW","GY","HT","HM","VA","HN","HK","HU","IS","IN","ID","IR","IQ","IE","IM","IL","IT","JM","JP","JE","JO","KZ","KE","KI","KP","KR","KW","KG","LA","LV","LB","LS","LR","LY","LI","LT","LU","MO","MG","MW","MY","MV","ML","MT","MH","MQ","MR","MU","YT","MX","FM","MD","MC","MN","ME","MS","MA","MZ","MM","NA","NR","NP","NL","NC","NZ","NI","NE","NG","NU","NF","MP","NO","OM","PK","PW","PS","PA","PG","PY","PE","PH","PN","PL","PT","PR","QA","MK","RO","RU","RW","RE","BL","SH","KN","LC","MF","PM","VC","WS","SM","ST","SA","SN","RS","SC","SL","SG","SX","SK","SI","SB","SO","ZA","GS","SS","ES","LK","SD","SR","SJ","SE","CH","SY","TW","TJ","TZ","TH","TL","TG","TK","TO","TT","TN","TR","TM","TC","TV","UG","UA","AE","GB","UM","US","UY","UZ","VU","VE","VN","VG","VI","WF","EH","YE","ZM","ZW","AX"]},"id_doc_type":{"type":["string","null"],"enum":["PASSPORT","ID_CARD","DRIVERS"]},"id_doc_front_file":{"type":["string","null"],"format":"uri","description":"Only image files are accepted (.jpg, .jpeg, .png)"},"id_doc_back_file":{"type":["string","null"],"format":"uri","description":"Only image files are accepted (.jpg, .jpeg, .png)"},"legal_name":{"type":["string","null"]},"alternate_name":{"type":["string","null"]},"formation_date":{"anyOf":[{"type":"string","format":"date-time"},{"type":"string"},{"type":"null"}],"format":"date-time"},"website":{"type":["string","null"],"format":"uri"},"owners":{"type":["array","null"],"items":{"type":"object","properties":{"role":{"type":"string","enum":["beneficial_controlling","beneficial_owner","controlling_person"],"description":"A beneficial owner is someone who directly or indirectly owns 25% or more of your business. A controlling person is someone with significant responsibility to control, manage or direct your business (typically a CEO, senior executive officer or equivalent).","example":"beneficial_owner"},"first_name":{"type":"string","minLength":1,"example":"John"},"last_name":{"type":"string","minLength":1,"example":"Doe"},"date_of_birth":{"anyOf":[{"type":"string","format":"date-time"},{"type":"string"}],"example":"1998-01-01T00:00:00Z"},"tax_id":{"type":"string","example":"536804398"},"address_line_1":{"type":"string","example":"738 Plain St"},"address_line_2":{"type":["string","null"],"example":"Building 22"},"city":{"type":"string","example":"Marshfield"},"state_province_region":{"type":"string","example":"MA"},"country":{"type":"string","enum":["AF","AL","DZ","AS","AD","AO","AI","AQ","AG","AR","AM","AW","AU","AT","AZ","BS","BH","BD","BB","BY","BE","BZ","BJ","BM","BT","BO","BQ","BA","BW","BV","BR","IO","BN","BG","BF","BI","CV","KH","CM","CA","KY","CF","TD","CL","CN","CX","CC","CO","KM","CD","CG","CK","CR","HR","CU","CW","CY","CZ","CI","DK","DJ","DM","DO","EC","EG","SV","GQ","ER","EE","SZ","ET","FK","FO","FJ","FI","FR","GF","PF","TF","GA","GM","GE","DE","GH","GI","GR","GL","GD","GP","GU","GT","GG","GN","GW","GY","HT","HM","VA","HN","HK","HU","IS","IN","ID","IR","IQ","IE","IM","IL","IT","JM","JP","JE","JO","KZ","KE","KI","KP","KR","KW","KG","LA","LV","LB","LS","LR","LY","LI","LT","LU","MO","MG","MW","MY","MV","ML","MT","MH","MQ","MR","MU","YT","MX","FM","MD","MC","MN","ME","MS","MA","MZ","MM","NA","NR","NP","NL","NC","NZ","NI","NE","NG","NU","NF","MP","NO","OM","PK","PW","PS","PA","PG","PY","PE","PH","PN","PL","PT","PR","QA","MK","RO","RU","RW","RE","BL","SH","KN","LC","MF","PM","VC","WS","SM","ST","SA","SN","RS","SC","SL","SG","SX","SK","SI","SB","SO","ZA","GS","SS","ES","LK","SD","SR","SJ","SE","CH","SY","TW","TJ","TZ","TH","TL","TG","TK","TO","TT","TN","TR","TM","TC","TV","UG","UA","AE","GB","UM","US","UY","UZ","VU","VE","VN","VG","VI","WF","EH","YE","ZM","ZW","AX"],"example":"US"},"postal_code":{"type":"string","example":"02050"},"id_doc_country":{"type":"string","enum":["AF","AL","DZ","AS","AD","AO","AI","AQ","AG","AR","AM","AW","AU","AT","AZ","BS","BH","BD","BB","BY","BE","BZ","BJ","BM","BT","BO","BQ","BA","BW","BV","BR","IO","BN","BG","BF","BI","CV","KH","CM","CA","KY","CF","TD","CL","CN","CX","CC","CO","KM","CD","CG","CK","CR","HR","CU","CW","CY","CZ","CI","DK","DJ","DM","DO","EC","EG","SV","GQ","ER","EE","SZ","ET","FK","FO","FJ","FI","FR","GF","PF","TF","GA","GM","GE","DE","GH","GI","GR","GL","GD","GP","GU","GT","GG","GN","GW","GY","HT","HM","VA","HN","HK","HU","IS","IN","ID","IR","IQ","IE","IM","IL","IT","JM","JP","JE","JO","KZ","KE","KI","KP","KR","KW","KG","LA","LV","LB","LS","LR","LY","LI","LT","LU","MO","MG","MW","MY","MV","ML","MT","MH","MQ","MR","MU","YT","MX","FM","MD","MC","MN","ME","MS","MA","MZ","MM","NA","NR","NP","NL","NC","NZ","NI","NE","NG","NU","NF","MP","NO","OM","PK","PW","PS","PA","PG","PY","PE","PH","PN","PL","PT","PR","QA","MK","RO","RU","RW","RE","BL","SH","KN","LC","MF","PM","VC","WS","SM","ST","SA","SN","RS","SC","SL","SG","SX","SK","SI","SB","SO","ZA","GS","SS","ES","LK","SD","SR","SJ","SE","CH","SY","TW","TJ","TZ","TH","TL","TG","TK","TO","TT","TN","TR","TM","TC","TV","UG","UA","AE","GB","UM","US","UY","UZ","VU","VE","VN","VG","VI","WF","EH","YE","ZM","ZW","AX"],"example":"BR"},"id_doc_type":{"type":"string","enum":["PASSPORT","ID_CARD","DRIVERS"],"example":"PASSPORT"},"id_doc_front_file":{"type":"string","format":"uri"},"id_doc_back_file":{"type":["string","null"],"format":"uri","description":"Only image files are accepted (.jpg, .jpeg, .png)"},"proof_of_address_doc_type":{"type":["string","null"],"enum":["UTILITY_BILL","BANK_STATEMENT","RENTAL_AGREEMENT","TAX_DOCUMENT","GOVERNMENT_CORRESPONDENCE"],"example":"UTILITY_BILL"},"proof_of_address_doc_file":{"type":["string","null"],"format":"uri","description":"Please upload one document that serve as proof of address, this needs to be the same address as the one provided above. Only PDF and image files are accepted (.jpg, .jpeg, .png, .pdf) with a maximum size of 3MB."},"ownership_percentage":{"type":["integer","null"],"minimum":0,"maximum":100,"description":"Ownership percentage of the beneficial owner (0-100)","example":25},"title":{"type":["string","null"],"maxLength":128,"description":"Job title of the owner","example":"CEO"},"tax_type":{"type":["string","null"],"enum":["SSN","ITIN"],"description":"Tax identifier type. Mandatory when country is US; must be SSN or ITIN.","example":"SSN"},"id":{"type":"string","minLength":15,"maxLength":15}},"required":["id"]}},"incorporation_doc_file":{"type":["string","null"],"format":"uri","description":"Please upload your articles of incorporation below. The articles of incorporation or a certificate of incorporation usually includes information like the company name, business purpose, number of shares offered, value of shares, etc. Only PDF and image files are accepted (.jpg, .jpeg, .png, .pdf) with a maximum size of 3MB."},"proof_of_ownership_doc_file":{"type":["string","null"],"format":"uri","description":"Please upload your proof of ownership below. This document usually includes information like the shareholders names and percentages. Only PDF and image files are accepted (.jpg, .jpeg, .png, .pdf) with a maximum size of 3MB."},"source_of_funds_doc_type":{"type":["string","null"],"enum":["business_income","gambling_proceeds","gifts","government_benefits","inheritance","investment_loans","pension_retirement","salary","sale_of_assets_real_estate","savings","esops","investment_proceeds","someone_else_funds"]},"source_of_funds_doc_file":{"type":["string","null"],"format":"uri"},"selfie_file":{"type":["string","null"],"format":"uri","description":"Only image files are accepted (.jpg, .jpeg, .png)"},"purpose_of_transactions":{"type":["string","null"],"enum":["business_transactions","charitable_donations","investment_purposes","payments_to_friends_or_family_abroad","personal_or_living_expenses","protect_wealth","purchase_good_and_services","receive_payment_for_freelancing","receive_salary","other"]},"purpose_of_transactions_explanation":{"type":["string","null"]},"account_purpose":{"type":["string","null"],"enum":["charitable_donations","ecommerce_retail_payments","investment_purposes","business_expenses","payments_to_friends_or_family_abroad","personal_or_living_expenses","protect_wealth","purchase_goods_and_services","receive_payments_for_goods_and_services","tax_optimization","third_party_money_transmission","payroll","treasury_management","other"]},"account_purpose_other":{"type":["string","null"],"maxLength":512,"description":"Required when account_purpose is \"other\". Max 512 characters."},"business_type":{"type":["string","null"],"enum":["corporation","llc","partnership","sole_proprietorship","trust","non_profit"]},"business_description":{"type":["string","null"],"maxLength":1000},"business_industry":{"type":["string","null"],"enum":["111998","112120","113310","115114","541211","541810","541430","541715","541930","561422","561311","561612","561740","561730","236115","236220","237310","238210","811111","812111","812112","532111","624410","541922","811210","812199","611110","611310","611410","611710","211120","212114","221310","562111","562920","213112","522110","522210","522320","523150","523940","523999","524113","813110","813211","813219","551112","721110","722511","722513","561510","713110","713210","712110","711110","711211","621111","621210","622110","623110","621511","623220","541940","621399","621910","541110","311421","337121","322220","339920","334210","339930","312130","334111","334118","325412","339112","336110","336390","315990","313110","339910","516120","513130","512250","519130","711410","711510","531110","531120","531130","531190","531210","531311","531312","531320","531390","454110","445110","455110","457110","449210","444110","459210","459120","445320","458110","458210","458310","455219","424210","456110","446120","541511","541512","541519","518210","511210","517111","517112","517410","481111","483111","485210","488510","484121","493110","423430","423690","423110","423830","423840","423510","424690","424990","424410","424480","423940","541611","541618","541330","541990","541214","561499"]},"estimated_annual_revenue":{"type":["string","null"],"enum":["0_99999","100000_999999","1000000_9999999","10000000_49999999","50000000_249999999","250000000_plus"]},"source_of_wealth":{"type":["string","null"],"enum":["business_dividends_or_profits","investments","asset_sales","client_investor_contributions","gambling","charitable_contributions","inheritance","affiliate_or_royalty_income"]},"publicly_traded":{"type":["boolean","null"]},"occupation":{"type":["string","null"],"maxLength":255},"external_id":{"type":["string","null"]},"tos_id":{"type":["string","null"],"minLength":15,"maxLength":15},"additional_info":{"type":["array","null"],"items":{"type":"object","properties":{"label":{"type":"string","enum":["EIN_LETTER","KYC_PROVIDER_DOCUMENT","FLOW_OF_FUNDS","BANK_STATEMENT","TAX_RETURN","AUDITED_FINANCIALS","OPERATING_AGREEMENT","BOARD_RESOLUTION","CERTIFICATE_OF_GOOD_STANDING","BUSINESS_LICENSE","MSB_LICENSE","POWER_OF_ATTORNEY","TRUST_DEED","SHAREHOLDER_REGISTRY","BENEFICIAL_OWNER_DECLARATION","COMPLIANCE_QUESTIONNAIRE","PROOF_OF_EMPLOYMENT","REFERENCE_LETTER","OTHER"]},"value":{"type":"string","minLength":1}},"required":["label","value"],"example":{"label":"EIN_LETTER","value":"https://example.com/ein.pdf"}},"description":"Optional extra documents or notes for this customer. `label` is a known category, `value` is either a URL to upload or a free-text description. Any URLs are downloaded and re-hosted on Blindpay storage before being stored."}},"required":["email","country"],"description":"Update a customer"}},"required":["instance_id","id"]},
    method: "put",
    pathTemplate: "/v1/instances/{instance_id}/customers/{id}",
    executionParameters: [{"name":"instance_id","in":"path"},{"name":"id","in":"path"},{"name":"Idempotency-Key","in":"header"}],
    requestBodyContentType: "application/json",
    securityRequirements: [{"Bearer":[]}]
  }],
  ["DeleteV1InstancesCustomersById", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."}},"required":["instance_id","id"]},
    method: "delete",
    pathTemplate: "/v1/instances/{instance_id}/customers/{id}",
    executionParameters: [{"name":"instance_id","in":"path"},{"name":"id","in":"path"},{"name":"Idempotency-Key","in":"header"}],
    requestBodyContentType: undefined,
  ...
  ["PostV1InstancesCustomersRfi", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"customer_id":{"type":"string","minLength":15,"maxLength":15,"description":"Customer ID"},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","additionalProperties":{},"description":"The JSON request body."}},"required":["instance_id","customer_id"]},
    method: "post",
    pathTemplate: "/v1/instances/{instance_id}/customers/{customer_id}/rfi",
    executionParameters: [{"name":"instance_id","in":"path"},{"name":"customer_id","in":"path"},{"name":"Idempotency-Key","in":"header"}],
    requestBodyContentType: "application/json",
  ...
  ["DeleteV1InstancesCustomersBankAccountsById", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"customer_id":{"type":"string","minLength":15,"maxLength":15,"description":"Customer ID"},"id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."}},"required":["instance_id","customer_id","id"]},
    method: "delete",
    pathTemplate: "/v1/instances/{instance_id}/customers/{customer_id}/bank-accounts/{id}",
    executionParameters: [{"name":"instance_id","in":"path"},{"name":"customer_id","in":"path"},{"name":"id","in":"path"},{"name":"Idempotency-Key","in":"header"}],
    requestBodyContentType: undefined,
    securityRequirements: [{"Bearer":[]}]
  }],
  ...
  ["PutV1InstancesCustomersVirtualAccountsById", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"customer_id":{"type":"string","minLength":15,"maxLength":15,"description":"Customer ID"},"id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"token":{"type":"string","enum":["USDC","USDT","USDB"]},"blockchain_wallet_id":{"type":"string","minLength":15,"maxLength":15}},"required":["token","blockchain_wallet_id"],"description":"Update a virtual account"}},"required":["instance_id","customer_id","id"]},
    method: "put",
    pathTemplate: "/v1/instances/{instance_id}/customers/{customer_id}/virtual-accounts/{id}",
    executionParameters: [{"name":"instance_id","in":"path"},{"name":"customer_id","in":"path"},{"name":"id","in":"path"},{"name":"Idempotency-Key","in":"header"}],
    requestBodyContentType: "application/json",
  ...
  ["PostV1InstancesCustomersBlockchainWallets", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"customer_id":{"type":"string","minLength":15,"maxLength":15,"description":"Customer ID"},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"name":{"type":"string"},"network":{"type":"string","enum":["base","sepolia","arbitrum_sepolia","base_sepolia","arbitrum","polygon","polygon_amoy","ethereum","stellar","stellar_testnet","tron","solana","solana_devnet"]},"signature_tx_hash":{"type":["string","null"]},"address":{"type":["string","null"]},"is_account_abstraction":{"type":["boolean","null"]}},"required":["name","network"],"description":"Create a new blockchain wallet"}},"required":["instance_id","customer_id"]},
    method: "post",
    pathTemplate: "/v1/instances/{instance_id}/customers/{customer_id}/blockchain-wallets",
    executionParameters: [{"name":"instance_id","in":"path"},{"name":"customer_id","in":"path"},{"name":"Idempotency-Key","in":"header"}],
    requestBodyContentType: "application/json",
  ...
  ["PostV1InstancesCustomersBankAccountsOfframpWallets", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"customer_id":{"type":"string","minLength":15,"maxLength":15,"description":"Customer ID"},"bank_account_id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"external_id":{"type":["string","null"]},"network":{"type":"string","enum":["tron","solana","solana_devnet","arbitrum","base","ethereum","polygon","sepolia","arbitrum_sepolia","base_sepolia","polygon_amoy"]}},"required":["network"],"description":"Create a new offramp wallet"}},"required":["instance_id","customer_id","bank_account_id"]},
    method: "post",
    pathTemplate: "/v1/instances/{instance_id}/customers/{customer_id}/bank-accounts/{bank_account_id}/offramp-wallets",
    executionParameters: [{"name":"instance_id","in":"path"},{"name":"customer_id","in":"path"},{"name":"bank_account_id","in":"path"},{"name":"Idempotency-Key","in":"header"}],
    requestBodyContentType: "application/json",
    securityRequirements: [{"Bearer":[]}]
  }],
  ["GetInstancesCustomersBankAccountsOfframpWalletsById", {
    name: "GetInstancesCustomersBankAccountsOfframpWalletsById",
  ...
  ["PostV1InstancesQuotesFx", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"from":{"type":"string","enum":["USDC","USDT","USDB"]},"to":{"type":"string","enum":["BRL","USD","MXN","COP","ARS","EUR"]},"request_amount":{"type":"number","minimum":500,"description":"1000 represents 10.00, 2050 represents 20.50"},"currency_type":{"type":"string","enum":["sender","receiver"],"description":""}},"required":["from","to","request_amount","currency_type"],"description":"Check FX rate"}},"required":["instance_id"]},
    method: "post",
  ...
  ["GetV1InstancesPayouts", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"limit":{"type":"string","enum":["10","50","100","200","500","1000"],"description":"Number of items to return"},"offset":{"type":"string","enum":["0","10","50","100","200","500","1000"],"description":"Number of items to skip"},"starting_after":{"type":"string","description":"A cursor for use in pagination. starting_after is an object ID that defines your place in the list."},"ending_before":{"type":"string","description":"A cursor for use in pagination. ending_before is an object ID that defines your place in the list."},"customer_id":{"type":"string","minLength":15,"maxLength":15},"status":{"type":"string","enum":["processing","failed","refunded","completed","on_hold"]},"customer_name":{"type":"string"},"bank_account_id":{"type":"string"},"country":{"type":"string"},"payment_method":{"type":"string","enum":["wire","ach","pix","pix_safe","ted","spei_bitso","transfers_bitso","ach_cop_bitso","international_swift","rtp","sepa"]},"network":{"type":"string"},"token":{"type":"string"}},"required":["instance_id"]},
    method: "get",
    pathTemplate: "/v1/instances/{instance_id}/payouts",
    executionParameters: [{"name":"instance_id","in":"path"},{"name":"limit","in":"query"},{"name":"offset","in":"query"},{"name":"starting_after","in":"query"},{"name":"ending_before","in":"query"},{"name":"customer_id","in":"query"},{"name":"status","in":"query"},{"name":"customer_name","in":"query"},{"name":"bank_account_id","in":"query"},{"name":"country","in":"query"},{"name":"payment_method","in":"query"},{"name":"network","in":"query"},{"name":"token","in":"query"}],
    requestBodyContentType: undefined,
    securityRequirements: [{"Bearer":[]}]
  }],
  ...
  ["PostV1InstancesPayoutsStellar", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"quote_id":{"type":"string","minLength":15,"maxLength":15},"signed_transaction":{"type":["string","null"],"description":"Signed transaction"},"sender_wallet_address":{"type":"string","description":"Payout wallet address"}},"required":["quote_id","sender_wallet_address"],"description":"Start payout on stellar blockchain"}},"required":["instance_id"]},
    method: "post",
  ...
  ["PostV1InstancesPayoutsSolana", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"quote_id":{"type":"string","minLength":15,"maxLength":15},"signed_transaction":{"type":["string","null"],"description":"Signed transaction"},"sender_wallet_address":{"type":"string","description":"Payout wallet address"}},"required":["quote_id","sender_wallet_address"],"description":"Start payout on solana blockchain"}},"required":["instance_id"]},
    method: "post",
  ...
  ["PostV1InstancesPayinQuotesFx", {
    ...
    description: `Get FX Rate`,
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"from":{"type":"string","enum":["USDC","USDT","USDB","BRL","USD","MXN","COP","ARS","EUR"]},"to":{"type":"string","enum":["USDC","USDT","USDB","BRL","USD","MXN","COP","ARS","EUR"]},"request_amount":{"type":"number","minimum":500,"description":"1000 represents 10.00, 2050 represents 20.50"},"currency_type":{"type":"string","enum":["sender","receiver"],"description":""}},"required":["from","to","request_amount","currency_type"],"description":"Check FX rate"}},"required":["instance_id"]},
    method: "post",
  ...
  ["GetV1InstancesPayins", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"limit":{"type":"string","enum":["10","50","100","200","500","1000"],"description":"Number of items to return"},"offset":{"type":"string","enum":["0","10","50","100","200","500","1000"],"description":"Number of items to skip"},"starting_after":{"type":"string","description":"A cursor for use in pagination. starting_after is an object ID that defines your place in the list."},"ending_before":{"type":"string","description":"A cursor for use in pagination. ending_before is an object ID that defines your place in the list."},"customer_id":{"type":"string","minLength":15,"maxLength":15},"status":{"type":"string","enum":["processing","on_hold","failed","refunded","completed"]},"customer_name":{"type":"string"},"bank_account_id":{"type":"string"},"country":{"type":"string"},"payment_method":{"type":"string","enum":["ach","wire","pix","ted","spei","transfers","pse","international_swift","rtp"]},"network":{"type":"string"},"token":{"type":"string"}},"required":["instance_id"]},
    method: "get",
  ...
  ["PostV1InstancesPayinsEvm", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"payin_quote_id":{"type":"string","minLength":15,"maxLength":15}},"required":["payin_quote_id"],"description":"Start payin"}},"required":["instance_id"]},
    method: "post",
  ...
  ["PostV1InstancesCustomersWallets", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"customer_id":{"type":"string","minLength":15,"maxLength":15,"description":"Customer ID"},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"network":{"type":"string","enum":["base","sepolia","arbitrum_sepolia","base_sepolia","arbitrum","polygon","polygon_amoy","ethereum","stellar","stellar_testnet","tron","solana","solana_devnet"]},"external_id":{"type":["string","null"],"maxLength":255},"name":{"type":"string","maxLength":255}},"required":["network","name"],"description":"Create a new wallet"}},"required":["instance_id","customer_id"]},
    method: "post",
    pathTemplate: "/v1/instances/{instance_id}/customers/{customer_id}/wallets",
    executionParameters: [{"name":"instance_id","in":"path"},{"name":"customer_id","in":"path"},{"name":"Idempotency-Key","in":"header"}],
    requestBodyContentType: "application/json",
  ...
  ["PostV1InstancesTransferQuotes", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"wallet_id":{"type":"string","minLength":15,"maxLength":15},"amount_reference":{"type":"string","enum":["sender","receiver"],"description":""},"cover_fees":{"type":["boolean","null"],"description":"If true, the sender will cover the fees. If false, the customer will cover the fees."},"request_amount":{"type":"number","minimum":1,"description":"100 represents 1, 2050 represents 20.50"},"sender_token":{"type":"string","enum":["USDC","USDT","USDB"]},"customer_wallet_address":{"type":"string","minLength":32,"maxLength":64},"customer_token":{"type":"string","enum":["USDC","USDT","USDB"]},"customer_network":{"type":"string","enum":["base","sepolia","arbitrum_sepolia","base_sepolia","arbitrum","polygon","polygon_amoy","ethereum","stellar","stellar_testnet","tron","solana","solana_devnet"]},"partner_fee_id":{"type":["string","null"],"minLength":15,"maxLength":15}},"required":["wallet_id","amount_reference","request_amount","sender_token","customer_wallet_address","customer_token","customer_network"],"description":"Create a new quote"}},"required":["instance_id"]},
    method: "post",
  ...
  ["PostV1InstancesTransfers", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"transfer_quote_id":{"type":"string","minLength":15,"maxLength":15}},"required":["transfer_quote_id"],"description":"Start transfer"}},"required":["instance_id"]},
    method: "post",
    pathTemplate: "/v1/instances/{instance_id}/transfers",
    executionParameters: [{"name":"instance_id","in":"path"},{"name":"Idempotency-Key","in":"header"}],
    requestBodyContentType: "application/json",
  ...
  ["PostV1UploadAnalyze", {
    name: "PostV1UploadAnalyze",
    description: `Analyze Document`,
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","description":"Instance ID. When provided, the request must be authenticated with an API key scoped to this instance. When omitted, a dashboard (Clerk) session is required."},"requestBody":{"type":"string","description":"Document to read and analyze with AI"}}},
    method: "post",
  ...
  ["PostV1InstancesMintUsdbSolana", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"address":{"type":"string"},"amount":{"type":"string"}},"required":["address","amount"],"description":"The JSON request body."}},"required":["instance_id"]},
    method: "post",
  ...
  ["PostV1InstancesCreateAssetTrustline", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"address":{"type":"string"}},"required":["address"],"description":"The JSON request body."}},"required":["instance_id"]},
    method: "post",
  ...
  ["PostV1InstancesMintUsdbStellar", {
    ...
    inputSchema: {"type":"object","properties":{"instance_id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"address":{"type":"string"},"amount":{"type":"string"},"signedXdr":{"type":"string"}},"required":["address","amount"],"description":"The JSON request body."}},"required":["instance_id"]},
    method: "post",
