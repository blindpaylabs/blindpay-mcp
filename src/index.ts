...
export const SERVER_VERSION = '1.6.5';
...
const toolDefinitionMap: Map<string, McpToolDefinition> = new Map([
  ["PutV1InstancesById", {
    ...
    inputSchema: {"type":"object","properties":{"id":{"type":"string","minLength":15,"maxLength":15},"Idempotency-Key":{"type":"string","maxLength":255,"description":"Optional key to safely retry this request without performing the action twice. Retrying with the same key and an identical body replays the original response; reusing a key with a different body returns an error. Keys are kept for 24 hours."},"requestBody":{"type":"object","properties":{"name":{"type":"string","minLength":3,"maxLength":100},"receiver_invite_redirect_url":{"type":["string","null"],"format":"uri"},"email_notifications":{"type":"boolean","default":true,"description":"Whether email notifications are enabled for this instance"},"require_passkey":{"type":"boolean","default":false,"description":"Whether passkey verification is required for protected actions"},"compliance_emails":{"type":["array","null"],"items":{"type":"string","format":"email"},"maxItems":20,"description":"When non-empty, compliance Request for Information emails are sent only to these addresses instead of all instance members"},"receiver_rfi_emails_enabled":{"type":"boolean","description":"Whether customer (receiver) Request for Information notification emails are sent for this instance. Omit to leave unchanged."}},"required":["name"],"description":"Update instance"}},"required":["id"]},
    method: "put",
